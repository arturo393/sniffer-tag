...
#define dwt_or8bitoffsetreg(addr, offset, or_val) dwt_modify8bitoffsetreg(addr, offset, (uint8_t)-1, or_val)
...
#define dwt_or16bitoffsetreg(addr, offset, or_val) dwt_modify16bitoffsetreg(addr, offset, (uint16_t)-1, or_val)
...
#define dwt_set_bit_num_16bit_reg(addr,bit_num) dwt_modify16bitoffsetreg(addr,0,0xffff,DWT_BIT_MASK(bit_num))
#define dwt_clr_bit_num_16bit_reg(addr,bit_num) dwt_modify16bitoffsetreg(addr,0,~DWT_BIT_MASK(bit_num),0)

#define dwt_or32bitoffsetreg(addr, offset, or_val) dwt_modify32bitoffsetreg(addr, offset, (uint32_t)-1, or_val)
...
#define dwt_set_bit_num_32bit_reg(addr,bit_num) dwt_modify32bitoffsetreg(addr,0,(uint32_t)-1,DWT_BIT_MASK(bit_num))
...
typedef enum {
	DWT_DGC_SEL_CH5 = 0, DWT_DGC_SEL_CH9
} dwt_dgc_chan_sel;

/* Enum used for selecting location to load DGC data from */
typedef enum {
	DWT_DGC_LOAD_FROM_SW = 0, DWT_DGC_LOAD_FROM_OTP
...
#define TX_ANT_DLY_HP (uint16_t) 16410
#define RX_ANT_DLY_HP (uint16_t) 16410
#define TX_ANT_DLY_LP (uint16_t) 16370
#define RX_ANT_DLY_LP (uint16_t) 16370
...
void dwt_writetodevice(uint32_t regFileID, uint16_t index, uint16_t length,
		uint8_t *buffer);
void dwt_xfer3000(const uint32_t regFileID, const uint16_t indx,
