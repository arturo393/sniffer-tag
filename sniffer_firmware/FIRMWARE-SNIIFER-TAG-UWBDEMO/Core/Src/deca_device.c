...
void dwt_configurestsmode(uint8_t stsMode)
{
    ...
    dwt_modify32bitoffsetreg(SYS_CFG_ID, 0, ~(SYS_CFG_CP_SPC_BIT_MASK | SYS_CFG_CP_SDC_BIT_MASK),
        (uint32_t)((uint16_t)stsMode & DWT_STS_CONFIG_MASK) << SYS_CFG_CP_SPC_BIT_OFFSET);
...
int dwt_readstsstatus(uint16_t* stsStatus, int sts_num)
{
    ...
    case DBL_BUFF_ACCESS_BUFFER_1:
        //!!! Assumes that Indirect pointer register B was already set. This is done in the dwt_setdblrxbuffmode when mode is enabled.
        *stsStatus = dwt_read16bitoffsetreg(INDIRECT_POINTER_B_ID, (uint16_t)(stsStatusRegAdd-BUF0_RX_FINFO+2)) >> 7;
        break;
    case DBL_BUFF_ACCESS_BUFFER_0:
...
void dwt_readdiagnostics(dwt_rxdiag_t *diagnostics)
{
    ...
    case DBL_BUFF_ACCESS_BUFFER_0:
        ...
        else
        {
            if (pdw3000local->cia_diagnostic & DW_CIA_DIAG_LOG_MAX)
            {
                dwt_readfromdevice((uint32_t)offset_buff, 0, DB_MAX_DIAG_SIZE, temp);
            }
            else if (pdw3000local->cia_diagnostic & DW_CIA_DIAG_LOG_MID)
            {
                dwt_readfromdevice((uint32_t)offset_buff, 0, DB_MID_DIAG_SIZE, temp);
            }
            else
            {
                dwt_readfromdevice((uint32_t)offset_buff, 0, DB_MIN_DIAG_SIZE, temp);
            }
        }
        ...
        diagnostics->ipatovPOA = (uint16_t)((uint16_t)temp[BUF0_IP_TS - BUF0_RX_FINFO + 2] << 8 | temp[BUF0_IP_TS - BUF0_RX_FINFO + 1]); // Phase of arrival as computed from the Ipatov CIR (signed rad*2-12)

        diagnostics->stsRxStatus = temp[BUF0_STS_TS - BUF0_RX_FINFO + CIA_C_STAT_OFFSET];       // RX status info for STS
        diagnostics->stsPOA = (uint16_t)((uint16_t)temp[BUF0_STS_TS - BUF0_RX_FINFO + 2] << 8 | temp[BUF0_STS_TS - BUF0_RX_FINFO + 1]); // Phase of arrival as computed from the STS 1 CIR (signed rad*2-12)

        diagnostics->sts2RxStatus = temp[BUF0_STS1_TS - BUF0_RX_FINFO + CIA_C_STAT_OFFSET];       // RX status info for STS1
        diagnostics->sts2POA = (uint16_t)((uint16_t)temp[BUF0_STS1_TS - BUF0_RX_FINFO + 2] << 8 | temp[BUF0_STS1_TS - BUF0_RX_FINFO + 1]); // Phase of arrival as computed from the STS 1 CIR (signed rad*2-12)
        ...
        diagnostics->ipatovFpIndex = (uint16_t)((uint16_t)temp[BUF0_IP_DIAG_8 - BUF0_RX_FINFO + 1] << 8 | temp[BUF0_IP_DIAG_8 - BUF0_RX_FINFO]);              // First path index [15:0] for Ipatov sequence
        ...
        diagnostics->stsPower = (uint16_t)((uint16_t)temp[BUF0_STS_DIAG_1 - BUF0_RX_FINFO + 1] << 8 | temp[BUF0_STS_DIAG_1 - BUF0_RX_FINFO]);           // channel area allows estimation of channel power for the STS
        ...
        diagnostics->sts2Power = (uint16_t)((uint16_t)temp[BUF0_STS1_DIAG_1 - BUF0_RX_FINFO + 1] << 8 | temp[BUF0_STS1_DIAG_1 - BUF0_RX_FINFO]);           // channel area allows estimation of channel power for the STS
    ...
    default:  //double buffer is off
        ...
        diagnostics->ipatovPOA = (uint16_t)((uint16_t)temp[IP_TOA_HI_ID - IP_TOA_LO_ID + 2] << 8 | temp[IP_TOA_HI_ID - IP_TOA_LO_ID + 1]); // Phase of arrival as computed from the Ipatov CIR (signed rad*2-12)
        ...
        diagnostics->stsPOA = (uint16_t)((uint16_t)temp[STS_TOA_HI_ID - IP_TOA_LO_ID + 2] << 8 | temp[STS_TOA_HI_ID - IP_TOA_LO_ID + 1]); // Phase of arrival as computed from the STS 1 CIR (signed rad*2-12)
        ...
        diagnostics->sts2POA = (uint16_t)((uint16_t)temp[STS1_TOA_HI_ID - IP_TOA_LO_ID + 2] << 8 | temp[STS1_TOA_HI_ID - IP_TOA_LO_ID + 1]); // Phase of arrival as computed from the STS 1 CIR (signed rad*2-12)
        ...
        diagnostics->ipatovFpIndex = (uint16_t)((uint16_t)temp[IP_DIAG_8_ID - IP_TOA_LO_ID + 1] << 8 | temp[IP_DIAG_8_ID - IP_TOA_LO_ID]);              // First path index [15:0] for Ipatov sequence
        ...
        diagnostics->stsPower = (uint16_t)((uint16_t)temp[STS_DIAG_1_ID - IP_TOA_LO_ID + 1] << 8 | temp[STS_DIAG_1_ID - IP_TOA_LO_ID]);           // channel area allows estimation of channel power for the STS
        ...
        diagnostics->sts2Power = (uint16_t)((uint16_t)temp[STS1_DIAG_1_ID - STS_DIAG_4_ID + offset_0xd + 1] << 8 | temp[STS1_DIAG_1_ID - STS_DIAG_4_ID + offset_0xd]);           // channel area allows estimation of channel power for the STS
...
int16_t dwt_readpdoa(void)
{
    ...
    if ((unsigned long)pdoa & B12_SIGN_EXTEND_TEST) pdoa |= B12_SIGN_EXTEND_MASK; //sign extend
    return pdoa;
}
...
void dwt_repeated_cw(int cw_enable, int cw_mode_config)
{
    ...
    dwt_write32bitoffsetreg(PG_TEST_ID, 0x0, (uint32_t)(cw_mode_config << ((cw_enable - 1) * 4)));
}
